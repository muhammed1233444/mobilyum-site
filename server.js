...
const { buttonHtml: favoriteButtonHtml } = require("./favorites.js");
...
app.get("/favorites.js", sendPublicFile("favorites.js", "public, max-age=604800"));
app.get("/favorites.css", sendPublicFile("favorites.css", "public, max-age=604800"));
...
const SEO_PAGES = {
"/begendiklerim": {title:"Beğendiklerim | Mobilyum Çorlu",description:"Beğendiğiniz Mobilyum ürünleri için toplu fiyat ve elden taksit bilgisi alın.",breadcrumb:"Beğendiklerim",h1:"Beğendiklerim",lead:"Eviniz için seçtikleriniz burada. İstediğiniz ürünleri işaretleyin, birlikte fiyat alalım.",heroImage:"/assets/optimized/mobilyum-corlu-og.jpg",imageAlt:"Mobilyum Çorlu",robots:"noindex, follow",pageClass:"seo-page-favorites",favorites:true},
  "/yatak-odasi": {
...
function renderPolicyBody(type) {
  if (type === "cookie") return `<section class="policy-content"><h2>Kullandığımız teknolojiler</h2><p><strong>Gerekli kayıtlar:</strong> Güvenlik, yönetim paneli oturumu ve ziyaretçinin çerez tercihini hatırlamak için kullanılır. Bunlar sitenin temel çalışması için gereklidir.</p><p><strong>İsteğe bağlı analiz:</strong> Yalnızca ziyaretçi izin verdiğinde; sayfa görüntüleme, yaklaşık oturum sayısı, mobil veya masaüstü cihaz türü, yönlendiren site ve WhatsApp, telefon ya da yol tarifi tıklamaları toplu istatistik olarak kaydedilir.</p><p>Analiz kayıtlarında ziyaretçinin adı, telefon numarası, mesaj içeriği veya kalıcı cihaz kimliği tutulmaz. Google Analytics ölçüm kimliği etkinleştirilirse Google Analytics de yalnızca analiz izni sonrasında yüklenir.</p><h2>Beğendiklerim</h2><p>Bir ürünü beğendiğinizde, listenizi bu tarayıcıyla eşleştiren <code>mobilyum_favorites</code> adlı gerekli çerez oluşturulur. Ürün kimlikleri, adları ve kapak bağlantıları sunucumuzda, bu rastgele anahtara bağlı bir listede tutulur. Liste ve çerez, son değişiklikten itibaren en fazla 1 yıl saklanır. Bu özellik için üyelik veya analiz izni gerekmez; beğeniler analiz amacıyla kullanılmaz. Ürünleri Beğendiklerim sayfasından kaldırabilirsiniz. Site çerezlerini silmek kayıtlı listeye bu tarayıcıdan erişimi kaldırır.</p><p>Toplu fiyat düğmesi seçtiğiniz ürün adları ve bağlantılarıyla bir WhatsApp mesajı hazırlar; gönderim sizin onayınızla WhatsApp üzerinden gerçekleşir.</p><h2>Tercihinizi değiştirme</h2><p>Sayfanın sol altındaki “Çerez ayarları” düğmesini kullanarak analiz izninizi dilediğiniz zaman geri çekebilir veya yeniden verebilirsiniz. Tercih kaydı tarayıcınızın yerel depolama alanında tutulur.</p><h2>İletişim</h2><p>Çerez kullanımıyla ilgili sorularınız için 0544 650 44 59 numarasından Mobilyum Çorlu'ya ulaşabilirsiniz.</p></section>`;
  return `<section class="policy-content"><h2>Veri sorumlusu ve iletişim</h2><p>Bu web sitesi bakımından veri sorumlusu Mobilyum Çorlu'dur. Adres: Reşadiye Mah. Şht. Teğmen Yavuzer Cad. No:53, 59850 Çorlu / Tekirdağ. Telefon: 0544 650 44 59.</p><h2>İşlenen bilgiler ve amaç</h2><p>Site performansını ve ilgi gören koleksiyonları anlamak amacıyla, yalnızca onay vermeniz hâlinde anonimleştirilmiş/toplu ziyaret istatistikleri işlenir. WhatsApp veya telefon bağlantısına tıkladığınızda görüşme ilgili uygulamada gerçekleşir; mesaj içeriğiniz bu web sitesindeki analiz sistemine kaydedilmez.</p><h2>Beğenilen ürün listesi</h2><p>Beğendiğiniz ürünler rastgele oluşturulan bir tarayıcı anahtarına bağlı olarak sunucumuzda saklanır. Üyelik, ad veya telefon numarası istenmez. Bu kayıtlar analiz sistemine aktarılmaz. Kayıt süresi ve kaldırma bilgileri için <a href="/cerez-politikasi">çerez politikasındaki Beğendiklerim bölümüne</a> bakabilirsiniz.</p><h2>Hukuki sebep ve tercihleriniz</h2><p>İsteğe bağlı analiz faaliyeti açık tercihinize dayanır. Onay vermemek sitenin ürünlerini incelemenizi veya iletişim bağlantılarını kullanmanızı engellemez. Tercihinizi çerez ayarlarından dilediğiniz zaman değiştirebilirsiniz.</p><h2>Haklarınız</h2><p>6698 sayılı Kişisel Verilerin Korunması Kanunu kapsamındaki talepleriniz için kimliğinizi doğrulayacak bilgilerle mağazamıza başvurabilir veya 0544 650 44 59 numarasından iletişime geçebilirsiniz.</p></section>`;
}

function renderSeoPageBody(page) {
if(page.favorites)return {products:[],html:"<section class=\"favorites-page\" data-favorites-page aria-label=\"Beğenilen ürünler\"><noscript><p>Beğendikleriniz için JavaScript özelliğini etkinleştirin.</p></noscript><div class=\"favorites-status\" data-favorites-status role=\"status\">Beğendikleriniz yükleniyor…</div><div class=\"favorites-layout\"><div><div class=\"favorites-toolbar\" data-favorites-toolbar hidden><span data-favorites-total></span><label><input type=\"checkbox\" data-favorites-all> Tümünü seç</label></div><div data-favorites-list></div></div><aside class=\"favorites-summary\" data-favorites-summary aria-label=\"Toplu fiyat isteği\"></aside></div><p class=\"favorites-storage-note\">Beğendikleriniz aynı cihaz ve tarayıcıda tekrar geldiğinizde hatırlanır. Listeye erişim için gerekli bir çerez kullanılır; son değişiklikten itibaren en fazla 1 yıl saklanır. Site çerezlerini silerseniz veya farklı tarayıcı kullanırsanız bu listeye erişemezsiniz. <a href=\"/cerez-politikasi\">Ayrıntılar</a></p></section>"};
...
function renderSeoPage(req, res) {
  ...
  const replacements = {
    HERO_MEDIA: page.favorites ? '' : '<img src="'+escapeHtml(page.heroImage)+'" width="1200" height="900" fetchpriority="high" decoding="async" alt="'+escapeHtml(page.imageAlt)+'">',
...
require('./favorites-server.js')(app,{storageDir:STORAGE_DIR,readProducts,safeProductImage});
